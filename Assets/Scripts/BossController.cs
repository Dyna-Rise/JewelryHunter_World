...
public class BossController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (hp > 0)
        {
            if (player != null)
            {
                ...
                //}
                animator.SetBool("InAttack", dist <= reactionDistance); //第二引数で基準の距離に入っているか、入っていないかを判断
            }
            else
            {
                animator.SetBool("InAttack", false);
        ...
        if (!inDamage)
        {
            if (collision.gameObject.tag == "Arrow") //ぶつかった相手がArrowだったら
            {
                ...
                if (hp <= 0)
                {
                    ...
                    //Boss Collide2つ持っているので、ある分だけゲットして配列に格納
    ...
    void Attack()
    {
        if (player != null)
        {
            ...
            //Xの差（底辺）、Ｙの差（高さ）から逆算して角度を求める ※ラジアン係数（円周率）で出てくる
            ...
            //（底辺と高さの差の情報は持っているが）あらためて長辺を1とした時の割合で底辺と高さを取得
    ...
    //SpriteRendererを無効化、3秒後にDestory
    void BossSpriteOff()
    {
